...
public class SendRequest extends DefaultClient {
	...
	public void serviceFound(ServiceReference servRef, SearchParameter search, String comManId) {
		try {
				// get operation
				Operation op = servRef.getService().getOperation(service, "DPWSOperation", null, null);

				// create input value and set string values
				ParameterValue request = op.createInputValue();
				ParameterValueManagement.setString(request, "lat", "17.4781737,78.2975116,15.51z");
				ParameterValueManagement.setString(request, "lon", "17.470432,78.3102378,15z");
